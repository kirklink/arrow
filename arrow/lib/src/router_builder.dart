...
class RouterBuilder {
  ...
  final _routeTree = <String, List<RouteBuilder>>{};
  ...
  /// A [RouterBuilder] is created to specify the URIs (routes) that the server can handle. Routers
  ...
  /// take a [Endpoint], which is the function to execute for the specified
  /// route.
  /// Middleware are executed in the following order:
  /// 1. Synchronous [RequestMiddleware] in the order they are added to the [RouterBuilder]
  /// 2. Asynchronous [RequestMiddleware] asynchronously in no guaranteed order until they are all completed
  /// 3. The request [Endpoint] which initiates the response
  /// 4. Asynchronous [ResponseMiddleware] asynchronously in no guaranteed order until they are all completed
  /// 5. Synchronous [ResponseMiddleware] in the order they are added to the [RouterBuilder]
  /// A [RouterBuilder] can also be created as a group, which is a sub-router for routes
  /// that have the same partial URIs. These groups can inherit or have their own
  /// middleware stack.
  /// A [RouterBuilder] can optionally be assigned a [Endpoint] to be used for not found routes
  /// (i.e. 404 errors) and/or a [Recoverer] from unhandled errors and exceptions.
  RouterBuilder(List<RouteBuilder> routes,
    ...
    for (final RouteBuilder route in routes) {
      _routeTree.putIfAbsent(route.method, () => <RouteBuilder>[]);
      _routeTree[route.method]!.add(route);
    }
  }
  ...
  // void route(Handler route) {}

  /// Create a GET route with the specified URI pattern and handler
  // RouteBuilder get(String pattern, Endpoint endpoint, {Pipeline? pipeline}) {
  //   pattern = _formatPattern(pattern);
  ...
  // RouteBuilder post(String pattern, Endpoint endpoint) {
  //   pattern = _formatPattern(pattern);
  //   RouteBuilder route = RouteBuilder(
  ...
  // RouteBuilder put(String pattern, Endpoint endpoint) {
  //   pattern = _formatPattern(pattern);
  //   RouteBuilder route = RouteBuilder(
  ...
  // RouteBuilder delete(String pattern, Endpoint endpoint) {
  //   pattern = _formatPattern(pattern);
  //   RouteBuilder route = RouteBuilder(
